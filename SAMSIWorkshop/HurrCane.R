...
p1 = ggplot(df.count,aes(x=year, y=count)) + 
...
f1 <- mws.sf$mws ~ year
m1 <- lm(f1,data=mws.sf)
summary(m1) # show results
plot(forecast(f1,5,level=c(95)))
...
fit1 <- lm(count ~ year, data = df.count)
summary(fit1)
p1+stat_smooth(method = "lm", col = "blue")
...
# Fit model
layout(matrix(c(1,2),1,2)) # optional 4 graphs/page
...
coeftest(fitARIMA2)
plot(forecast(fitARIMA2,20, level = c(90)))
...
plot(forecast(fitARIMA2,15, level = c(95)))
